...
void GraphicsManager::SetWindow(const HWND& Wnd)
{
	mWnd = Wnd;
}
...
HRESULT GraphicsManager::CompileShaderFromFile(const WCHAR* const szFileName, const LPCSTR szEntryPoint, const LPCSTR szShaderModel, ID3DBlob** const ppBlobOut) const
{
...
HRESULT GraphicsManager::InitDXDevice()
{
	HRESULT hr = static_cast<HRESULT>(0L);
...
HRESULT GraphicsManager::Render(const std::vector<GameObject>& entities, const Camera* const camera, const float& time)
{
	HRESULT hr = static_cast<HRESULT>(0L);
	...
	mImmediateContext->ClearRenderTargetView(mRenderTargetView, Colors::Black);
	...
	//test windown imgui
	ImGui::Begin("ImGUI");
	//For reseting the rocket imgui commands
...
void GraphicsManager::SetLights(const Light& light)
{
...
void GraphicsManager::SetCamera(const Camera* const camera)
{
...
HRESULT GraphicsManager::VertexIndex(const Mesh& shape)
{
	HRESULT hr = static_cast<HRESULT>(0L);
...
HRESULT GraphicsManager::Texture(const Mesh& shape)
{
	HRESULT hr = static_cast<HRESULT>(0L);
...
HRESULT GraphicsManager::Shader(const Mesh& shape)
{
	HRESULT hr = static_cast<HRESULT>(0L);
...
const int& GraphicsManager::Width() const
{
	return mWidth;
}


const int& GraphicsManager::Height() const
{
	return mHeight;
}
