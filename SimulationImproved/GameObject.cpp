...
void GameObject::CreateShape(const MeshType& geometryType, const bool& skyBox, const XMFLOAT3& translation, const XMFLOAT3& rotation, const XMFLOAT3& scale, 
	...
	const std::vector<InstanceData>* const instanceData, const bool& particleSystem)
{
	...
	case MeshType::CUBE:
	{
		const Mesh Shape(MeshType::CUBE, transform, translation, rotation, scale, diffuseTexture, normalMap, displacementMap, shader, name, instanceData, particleSystem);
	...
	{
		const Mesh Shape(MeshType::SPHERE, transform, translation, rotation, scale, diffuseTexture, normalMap, displacementMap, shader, name, instanceData, particleSystem);
		shapes.push_back(Shape);
		break;
	}
	...
	{
		const Mesh Shape(MeshType::CYLINDER, transform, translation, rotation, scale, diffuseTexture, normalMap, displacementMap, shader, name, instanceData, particleSystem);
		shapes.push_back(Shape);
		break;
	}
	...
	case MeshType::CONE:
	{
		const Mesh Shape(MeshType::CONE, transform, translation, rotation, scale, diffuseTexture, normalMap, displacementMap, shader, name, instanceData, particleSystem);
		shapes.push_back(Shape);
		break;
	}
	...
	case MeshType::QUAD:
	{
		const Mesh Shape(MeshType::QUAD, transform, translation, rotation, scale, diffuseTexture, normalMap, displacementMap, shader, name, instanceData, particleSystem);
		shapes.push_back(Shape);
		break;
	}
	}
...
void GameObject::AddLight(const DirectX::XMFLOAT4& translation, const DirectX::XMFLOAT4& colour, const std::string& name)
{
	const Light Light(transform, translation, colour, name);
	lights.push_back(Light);
}

const vector<Mesh>& GameObject::Shapes() const
{
	return shapes;
}


const std::vector<Light>& GameObject::Lights() const
{
	return lights;
}
...
void GameObject::MoveEntity(const DirectX::XMFLOAT3& translation, const float& timeScale, const float& dt)
{
...
void GameObject::RotateEntity(const DirectX::XMFLOAT3 & rotation, const float& timeScale, const float & dt)
{
...
void GameObject::RotateAroundPoint(const DirectX::XMFLOAT3& point, const DirectX::XMFLOAT3& rotation, const float& timeScale, const float& dt)
{
